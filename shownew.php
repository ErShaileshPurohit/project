...
<div class="content">
	...
	<?php $ab=$_GET['movie_name'];
	echo $ab;



	$s=$obj->select('add_movie',null,"movie_name='$ab'");


	 // $s=mysqli_query($conn,"select * from add_movie where movie_name='$ab'");
...
$t=$obj->select('add_theatre',null,"theatre_id='".$shw['t_id']."'");
...
<td>
<?php $tr=$obj->select('add_movie',null,"movie_name='".$shw['movie_name']."' AND t_id='".$shw['
t_id']."'");
...
// {
	...
	$ttm=$obj->select('show_time',null,"screen_id='".$shh['screen_id']."'");
	...
	?>
	<a href="seat.php?show=<?php echo $ttme['st_id'];?>&theatre=<?php echo $shw['t_id'];?>&screen=<?php echo $shh['screen_id'];?>&date=<?php echo $sent?>"><button class="btn btn-default mr-3"><?php echo date('h:i A',strtotime($ttme['start_time']));?></button></a>
	<?php
}}
